...
def calc_NMSE(x_hat,x_test,T=10,pow_diff=None):
    ...
    if T == 1:
        ...
        x_test_real = np.reshape(x_test[:, :,0, :, :], (x_test.shape[0]*x_test.shape[1], -1))
        x_test_imag = np.reshape(x_test[:, :,1, :, :], (x_test.shape[0]*x_test.shape[1], -1))
        x_test_temp = x_test_real + 1j*(x_test_imag)
    ...
    if T != 1:
        for t in range(1,T):
        #for t in range(T):
            x_test_temp =  np.reshape(x_test[:, t, :, :, :], (len(x_test), -1))
            x_hat_temp =  np.reshape(x_hat[:, t, :, :, :], (len(x_hat), -1))
            power = np.sum(abs(x_test_temp)**2, axis=1)
            mse = np.sum(abs(x_test_temp-x_hat_temp)**2, axis=1)
            temp = mse/power
